...
class BaseLoginProvider {
     ...
     * @return {?}
     */
    loadScript(id, src, onload, async$$1 = true, inner_text_content = '') {
        if (document.getElementById(id)) {
            return;
        }
        let /** @type {?} */ signInJS = document.createElement('script');
        signInJS.async = async$$1;
        signInJS.src = src;
...
class GoogleLoginProvider extends BaseLoginProvider {
    ...
    signIn(opt) {
        return new Promise((resolve, reject) => {
            this.onReady().then(() => {
                const /** @type {?} */ offlineAccess = (opt && opt.offline_access) || (this.opt && this.opt.offline_access);
                let /** @type {?} */ promise = !offlineAccess ? this.auth2.signIn(opt) : this.auth2.grantOfflineAccess(opt);
        ...
        return new Promise((resolve, reject) => {
            this.onReady().then(() => {
                if (IN.User.isAuthorized()) {
                    IN.API.Raw(`/people/~:(${this.fields})`).result((res) => {
                        let /** @type {?} */ user = new SocialUser();
                        user.id = res.id;
